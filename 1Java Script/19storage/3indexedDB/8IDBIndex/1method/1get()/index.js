...
request.onsuccess = (e) => {
    ...
    // getting object from the index using key
    indexRequest = index.get('vijayarangan')
    indexRequest.onsuccess = (e) => {
        console.log(indexRequest)
        list.innerHTML += `<li>the age of the given key using index is ${indexRequest.result.age}</li>`
    }

    indexRequest.onerror = (e) => {
        list.innerHTML += `<li>failed to get name from the index: ${indexRequest.error}</li>`
    }
}
