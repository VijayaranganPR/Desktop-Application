...
request.onsuccess = (e) => {
    ...
    indexRequest = index.count()

    indexRequest.onsuccess = (e) => {
        console.log(indexRequest)
        list.innerHTML += `<li>number of objects in nameIndex is ${indexRequest.result}</li>`
    }

    indexRequest.onerror = (e) => {
        list.innerHTML += `<li>failed to get count of index in nameIndex: ${indexRequest.error}</li>`
    }
